...
contract TSwapPool is ERC20 {
    ...
    {
        // @audit-info - zero address check is missing
    ...
    function deposit(
        ...
        // @audit -high?, not being used
        // if someone sets a deadline of next block, someone can still deposit!?
        // IMPACT: HIGH a user who expects a deopsit to fail, will go through. Sever impact on functionality
        // LIKLIHOOD: HIGH - always the case
        uint64 deadline
    )
        external
        ...
        returns (uint256 liquidityTokensToMint)
    {
        if (wethToDeposit < MINIMUM_WETH_LIQUIDITY) {
            // @audit-info - MINIMUM_WETH_LIQUIDITY is a constant and not required to be emitted
            revert TSwapPool__WethDepositAmountTooLow(MINIMUM_WETH_LIQUIDITY, wethToDeposit);
        }
        if (totalLiquidityTokenSupply() > 0) {
            uint256 wethReserves = i_wethToken.balanceOf(address(this));
            // e this seems to have been turned into a helper variable. No longer needed.
            // @audit-info / gas - dont need this
            ...
            // e if we calculate too many pool tokens to deposit, we should revert
            ...
            // @audit-info - it would be better if this was before _addLiquidityMintAndTransfer call to follow CEI pattern
        ...
        private
    {
        ...
        // LIKLIHOOD: HIGH - always the case
        // LiquidityAdded(msg.sender, wethToDeposit, poolTokensDeposited);
